...
const History: React.FC = () => {
  ...
  const refreshHistory = () => {
    const data = localStoreService.getAll<Product>('history');
  ...
  const handleRemove = (id: string) => {
    localStoreService.remove<Product>('history', id);
    refreshHistory();
  };

  // Xóa toàn bộ lịch sử
  const handleClear = () => {
    localStoreService.clear('history');
    refreshHistory();
  };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 32px 32px 32px' }}>
      ...
      {history.length === 0 ? (
        <Empty description="Chưa có lịch sử xem" style={{ margin: '80px 0' }} />
      ) : (
        <List
